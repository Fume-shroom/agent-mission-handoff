...
// codexTargetVersion identifies a translated rollout as AMH-produced.
const codexTargetVersion = "amh-handoff"
...
func ToCodex(s AgentSession, targetModelProvider ...string) ([]byte, string, error) {
	sessionID := DeterministicID(handoffSourceKey(s), "codex")
	turns := turnsWithPreamble(s)
	modelProvider := "openai"
	if len(targetModelProvider) > 0 && targetModelProvider[0] != "" {
		modelProvider = targetModelProvider[0]
	}
